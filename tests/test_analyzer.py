...
@pytest.mark.parametrize(
    ("name", "status", "actionable"),
    [
        ("match", "MATCH", False),
        ("mismatch", "DRIFT", True),
        ("cff_missing", "CFF_MISSING", False),
        ("cff_version_missing", "CFF_VERSION_MISSING", False),
        ("dynamic_pyproject", "MANIFEST_DYNAMIC", False),
        ("workspace_cargo", "SCOPE_AMBIGUOUS", False),
        ("ambiguous_manifests", "MANIFEST_AMBIGUOUS", False),
        ("description", "MATCH", False),
        ("julia", "MATCH", False),
        ("node", "MATCH", False),
        ("dart", "SCOPE_AMBIGUOUS", False),
    ],
)
